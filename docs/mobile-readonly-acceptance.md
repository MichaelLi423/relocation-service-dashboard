...
同日，手机浏览器 4 项 E2E 与发布 P1/P2/P3 均通过（分别运行，无跳过）。P2 验证停服期间本地改名、补录正常，按同一 origin 重启服务后自动发布恢复，并通过真实 UI 删除批次后确认新快照排除该记录。P3 验证原子信封、当前候选幂等、过期期望版本拒绝及版本 8 重启恢复。类型检查、207 项聚焦测试及 120 项既有桌面/IPC 回归通过；Electron 打包、独立 Web/服务构建和本地 Docker 构建通过。上表为**本地合成数据 E2E 验收**；生产侧于 2026-09-09 首次上线（凭证/首容器/loopback 角色/HTTP-01 证书/TLS/首份受管空冒烟），2026-09-11 完成一次真实凭证轮换并经用户实机确认业务发布（见 `docs/mobile-readonly-deployment.md` §8/§9/§10 与本文 §8），**不**属于这些本地证据，也不因此宣称本 change 生产发布整体完结。
...
- 2026-09-11 凭证轮换：经人工授权用独立工具 `scripts/rotate-mobile-readonly.py rotate` 执行一次真实
  轮换（退出码 0、`phase=complete`），仅替换自有凭证摘要并重启自有容器，未写业务、未换快照；
  旧查看/上传凭证失效、新凭证生效、职责隔离与 TLS 正常；恢复材料保留。详见部署文档 §10。
- 2026-09-11 用户实机确认：Windows 首次真实业务上传与实体手机读取已完成，真机 1–6 项全部通过
  （三时间展示；前台检查相邻触发 ≤60s；断网保留已展示数据且完整刷新不可用；联网恢复自动回前台；
  PC 断网仍可查看；新版本自动更新详情一致）。用户未提供设备型号/浏览器/精确耗时，本文不作
  量化实测声明，也不将本地 5 分钟预算结果外推为公网实测。
- 仍待确认：轮换后 Windows 自动发布与实体手机用新密码恢复的客户端确认（openspec 任务 9.3 保持未勾选）。
