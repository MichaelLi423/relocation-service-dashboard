...
同日，手机浏览器 4 项 E2E 与发布 P1/P2/P3 均通过（分别运行，无跳过）。P2 验证停服期间本地改名、补录正常，按同一 origin 重启服务后自动发布恢复，并通过真实 UI 删除批次后确认新快照排除该记录。P3 验证原子信封、当前候选幂等、过期期望版本拒绝及版本 8 重启恢复。类型检查、207 项聚焦测试及 120 项既有桌面/IPC 回归通过；Electron 打包、独立 Web/服务构建和本地 Docker 构建通过。上表为**本地合成数据 E2E 验收**；生产侧于 2026-09-09 首次上线（凭证/首容器/loopback 角色/HTTP-01 证书/TLS/首份受管空冒烟），2026-09-11 完成两轮真实凭证轮换（第一轮 `7a2e…`、第二轮 `a7c9daf8231e53117a753c6eefa4b806`）并经用户实机确认业务发布（见 `docs/mobile-readonly-deployment.md` §8/§8.1/§9/§10 与本文 §8），**不**属于这些本地证据，也不因此宣称本 change 生产发布整体完结。
...
- 除 `e2e/`、两个 playwright 配置、`package.json` 脚本、`tsconfig.json` include 与本
  文档外，不修改 `src/main|server|mobile|shared` 与既有测试（并发实现方：桌面/云端服务/手机各自负责）。
- safeStorage 使用真实 OS 后端，不提供明文 mock；凭证只写摘要（服务端）或密文
  （桌面 token 文件），UI/IPC 不回显任何 secret。
- 无真机/真实网络依赖；云端协议断言在本地真实 HTTPS 服务上进行。生产环境的用户实机确认
  （Windows 真实发布与实体手机读取）不属于本地 E2E，记录于本文 §8 与部署文档 §10。

## 8. 生产状态（2026-09-11，供部署负责人对照，非本仓库 E2E 证据）

- 生产首次上线（2026-09-09）：凭据（Keychain 两条目 + 远端仅 digest）、首个受限容器、loopback
  六角色、HTTP-01 独立证书、TLS 激活、生产 390px 截图空 V1 与 10 项 DOM 断言通过；
  详见 `docs/mobile-readonly-deployment.md` §8/§9。
- 2026-09-11 凭证轮换：经人工授权用独立工具 `scripts/rotate-mobile-readonly.py rotate` 完成两轮真实
  轮换（第一轮退出码 0、`phase=complete`；第二轮事务 `a7c9daf8231e53117a753c6eefa4b806`），仅替换
  自有凭证摘要并重启自有容器，未写业务、未换快照；旧查看/上传凭证失效、新凭证生效、职责隔离与
  TLS 正常；恢复材料保留。详见部署文档 §10。
- 2026-09-11 用户实机确认：Windows 首次真实业务上传与实体手机读取已完成，真机 1–6 项全部通过
  （三时间展示；前台检查相邻触发 ≤60s；断网保留已展示数据且完整刷新不可用；联网恢复自动回前台；
  PC 断网仍可查看；新版本自动更新详情一致）。轮换后 Windows 以新 upload token 恢复发布、手机以
  新 viewer 恢复查看，经用户/Coding Agent 确认。**证据标签：[用户/Coding Agent 确认，非仓库自动化
  证据]**；非 E2E、非可重放自动测试。用户未提供设备型号/浏览器/精确耗时，本文不作量化实测声明，
  也不将本地 5 分钟预算结果外推为公网实测。
- 生产当前为真实业务快照（非首版空 V1）；**未发生** DNS 切换与旧服务迁移（`workbench.michaelli.site`
  自始独立解析），也未创建同域 1Panel GUI 站点/证书条目。
- 权威 DNS 已实测（A `8.162.13.22`、TTL 600 权威应答非缓存、80/443 外部可达，见部署文档 §8 DNS 行）；
  `workbench.michaelli.site` 自始为独立解析，本 change 未做 DNS 切换，也不存在旧服务迁移；
  本运行未读取安全组规则 API、无任何写入；公网无需放行 8082（仅 127.0.0.1 loopback 反代）。
- 私密取回命令 `security find-generic-password … -w` 只在本机受信终端执行（会打印明文），
  禁止粘贴到聊天/仓库/共享环境；密钥与业务值不写入本文件。
