...
## 8. 生产状态（2026-09-09，供部署负责人对照，非本仓库 E2E 证据）

- 生产已上线（部分）：凭据（Keychain 两条目 + 远端仅 digest）、首个受限容器、loopback
  六角色、HTTP-01 独立证书、TLS 激活、生产 390px 截图空 V1 与 10 项 DOM 断言通过；
  详见 `docs/mobile-readonly-deployment.md` §8/§9。
- 仍待人工：Windows 首次**真实业务**发布（发布 URL origin + upload token 一次性配置→显式启用）、
  实体手机交付、凭证轮换（部署助手无自动化轮换命令且未演练，需人工授权后执行）。
