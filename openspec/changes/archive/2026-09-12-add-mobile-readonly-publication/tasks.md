...
- [x] 9.1 预上线定向解析测试：仅以新子域名 `workbench.michaelli.site` 预先定向解析做验证测试，`michaelli.site`/`www` 既有记录与既有站点保持不变；本地/测试不接触真实业务域名流量
- [x] 9.2 生产部署闸门核对（不自动通过、另行授权后执行）：公网安全组放行、`workbench.michaelli.site` A 记录（8.162.13.22，TTL 10 分钟）、1Panel 反代站点 + 独立 LE 证书、OpenResty 容器到内部服务（8082 为待检查候选）网络可达性逐项实际验收并记录
  - 实测口径（2026-09-09，权威证据）：独立新子域权威 A `8.162.13.22`、TTL 600（权威应答非缓存，无 AAAA/CNAME）；外部 HTTP/HTTPS 真实可达；同域以受控 OpenResty vhost + 原生 acme.sh 独立证书托管，后端仅 loopback `127.0.0.1:8082` 反代。**未**读取/改动安全组规则 API，未做 DNS/安全组/防火墙写入；同域 1Panel GUI 站点/证书条目**未**创建（不虚构 GUI 行为）。
- [x] 9.3 生产凭证生成与轮换：查看 Basic Auth 强密码 + 独立上传 token，二者以摘要落盘并演练职责隔离（上传不能读业务、查看不能写）；桌面上传 token 走 OS 安全存储一次性配置；不含 root/SSH 密钥
  - 证据（2026-09-11）：两轮真实轮换（第一轮 `7a2e614374318c85759e7ef35112db68`、第二轮 `a7c9daf8231e53117a753c6eefa4b806`），仅替换自有摘要并重启自有容器；轮换后 Windows 以新 upload token 恢复发布、手机以新 viewer 恢复查看。来源标签：**[用户/Coding Agent 确认，非仓库自动化证据]**；非 E2E、非可重放自动测试；不含明文/digest。
- [x] 9.4 生产发布执行：构建/推送镜像、启动服务、发布首份脱敏冒烟快照、手机真实浏览器验收（dataAsOf/publishedAt/lastCheckedAt、断网语义、版本重载提示）后切换 DNS；旧服务保留至缓存过渡期结束，云端服务本就不保留业务快照历史，不宣称保留生产历史快照
  - 实测口径（2026-09-09 起，2026-09-11 用户实机确认）：镜像为 linux/amd64 并经 `docker save | ssh docker load` 传输（**非** registry push）；完成受管空冒烟与真实业务发布、手机真实浏览器/真机验收。证据标签：**[用户/Coding Agent 确认，非仓库自动化证据]**；不作量化实测声明。
  - **未发生** DNS 切换与旧服务迁移（`workbench.michaelli.site` 自始独立解析）；「切换 DNS / 旧服务保留缓存过渡」标为 **N/A**，不属本 change 实际执行项。
- [x] 9.5 验证本 change 未改动正式基线 `openspec/specs/`、未触碰 `docs/issue/`、`docs/training/vibe-coding/` 与真实客户文件；交付最终 diff 供复核

> 进度说明（2026-09-11 更新；2026-09-09 历史状态分段保留；实际生产证据见
> `docs/mobile-readonly-deployment.md` §8/§9/§10 与 `docs/mobile-readonly-acceptance.md` §8。
> 勾选只代表「该步已完成或经部署负责人确认」的事实，不代表 OpenSpec 生产发布整体完结）：
> - 9.1 已勾选（2026-09-09）：定向解析/HTTP-01 challenge 验证通过（host=容器=外部一致，80→308→HTTPS），
>   未改动 `michaelli.site`/`www` 记录与既有站点；本 change 范围内无任何 DNS 写入。
> - 9.2 已勾选（2026-09-09 权威证据）：A `8.162.13.22`、TTL 600 为权威应答（NS
>   `dns31.hichina.com`=120.76.107.59，RD=0/AA=True，非缓存），AAAA/CNAME 均无；80/443 外部真实可达
>   （外部 HTTP-01 challenge + 正常 HTTPS 浏览器）；1Panel OpenResty 独立 vhost + 原生 acme.sh 托管、
>   OpenResty→127.0.0.1:8082 反代功能可用（同域 GUI 站点/证书条目**未**创建，不虚构 GUI API 行为）；
>   本运行未读取安全组规则 API、无任何 DNS/安全组/防火墙写入，公网无需放行 8082。
> - 9.3 已勾选（2026-09-11）。历史（2026-09-09）：本机 Keychain 两条目已建、远端仅存 scrypt 摘要且上传/查看
>   职责隔离已演练；当时部署助手无自动化轮换命令。新证据（2026-09-11）：已按人工授权用独立工具
>   `scripts/rotate-mobile-readonly.py rotate` 完成**两轮**真实轮换（第一轮 `7a2e614374318c85759e7ef35112db68`、
>   第二轮 `a7c9daf8231e53117a753c6eefa4b806`），仅替换自有摘要并重启自有容器；轮换后 Windows 以新
>   upload token 恢复发布、手机以新 viewer 恢复查看。**证据标签：[用户/Coding Agent 确认，非仓库自动化证据]**；
>   非 E2E、非可重放自动测试；不含明文/digest。原任务文字未削弱。
> - 9.4 已勾选（2026-09-11，基于用户实机确认）。历史（2026-09-09）：生产容器/独立证书/首份**受管空冒烟**
>   （CAS 0→V1、空集合、受管 SMOKE_ID）已执行。新证据（2026-09-11）：用户实机确认 Windows 首次真实上传与
>   实体手机读取，真机 1–6 项全部通过（三时间展示；前台检查相邻触发 ≤60s；断网保留已展示数据且完整刷新
>   不可用；联网恢复自动回前台；PC 断网仍可查看；新版本自动更新详情一致）。**证据标签：[用户/Coding Agent
>   确认，非仓库自动化证据]**；用户未提供设备型号/浏览器/精确耗时，故不作量化实测声明。生产当前为真实业务
>   快照（不再是首版空 V1）。实测口径：镜像为 linux/amd64 并经 `docker save | ssh docker load` 传输
>   （**非** registry push）；**未发生** DNS 切换与旧服务迁移（`workbench.michaelli.site` 自始独立解析），
>   该迁移项标为 **N/A**；同域未创建 1Panel GUI 站点/证书条目（以 OpenResty vhost + 原生 acme.sh 托管）。
> - 9.5 已勾选（部署负责人确认 diff 边界）：未改动 `openspec/specs/`，未触碰 `docs/issue/`、
>   `docs/training/vibe-coding/` 与真实客户文件。
>
> 进度：46/46 已勾选。勾选反映「任务项已完成/经确认」的记录事实，不改变各条证据来源性质
> （本地自动化 vs [用户/Coding Agent 确认，非仓库自动化证据]）。
