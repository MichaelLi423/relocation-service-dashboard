...
- 显式启用即时检查：负责人显式启用时，除既有「启动检查一次 + 运行中约每 2 分钟检查」外，SHALL 立即受控执行一次 single-flight 检查，并在 IPC 返回前给出该次 success/failure 状态（等待有界，不取消、不重复既有周期）；不承诺固定总完成时限。
...
- 实施口径（2026-09-09/2026-09-11）：同域实际以受控 OpenResty vhost + 原生 acme.sh 独立证书托管，**未**创建 1Panel GUI 同域站点/证书条目；**未发生** DNS 切换与旧服务迁移（`workbench.michaelli.site` 自始独立解析）；后端仅 loopback `127.0.0.1:8082`。
- 客户端升级/交付形态：Windows 交付使用 Electron Forge Squirrel Setup（当前版本 0.1.1）进行**手动安装/覆盖升级**；当前未实现 `autoUpdater` 自动更新，本 change 不新增 portable 形态（MakerZIP 仅用于 macOS/Linux 开发机验证）。
...
方案：凭证轮换按**显式事务**组织，状态与恢复材料保存在本机受管轮换事务目录
（`~/.cache/relocation-mobile-readonly/rotations`，mode 0700，含固定内容 marker `.rotate-managed` 与
`.lock`）并记录阶段供人工判定；远端受管控制资源为 `/opt/.relocation-mobile-readonly-rotate`
（mode 0700，marker `.rotate-managed`、`lock`、`.active-txn`）。变更仅限**自有可信资源**：本机受管事务目录、
远端受管控制资源、自有服务的凭证摘要文件 `credentials.json` 与自有 Keychain 条目。

- 变更方式：以 compare-and-swap 只替换**既有**的自有 `credentials.json` 摘要，并仅重启自有容器；
  工具自身另创建/使用其 marker、lock、active 与临时事务资源。不触碰主站、既有 vhost/证书、DNS、
  安全组、防火墙、镜像或业务快照。
- 核对方式：验证只使用快照哈希/非业务元数据与角色状态，不读取或传输业务正文。
- 凭据材料边界：**明文** secret 绝不进入普通文件、日志、文档或命令输出（仅短暂存在于内存与 OS Keychain，
  以及事务期间受管的临时 Keychain 旧/新条目）；**旧/新摘要** `credentials.json` 的精确字节仅作为
  **恢复材料**，以 mode 0600 存于本地受管事务目录（mode 0700）内；远端凭证文件按服务设计只存摘要
  （mode 0600、owner 1000:1000）；这些材料绝不对外暴露、不打印、不写入文档或聊天。
- 一致性边界：本机 Keychain、远端摘要与容器重启**跨系统不构成原子事务**；因此不自动重试、不自动回滚，
  `resume`/`rollback` 均需人工授权，异常时以只读 `status` 判定后再处置。
- 恢复材料保留：失败或状态不确定时保留恢复材料与临时条目，供人工授权后 `resume`/`rollback`。

理由：单负责人场景下以显式事务 + 人工判定换取可恢复与最小权限；跨系统原子性不可得，宜保守而非自动收敛。
