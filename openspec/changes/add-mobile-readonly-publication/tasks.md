...
> - 9.3 保持未勾选。历史（2026-09-09）：本机 Keychain 两条目已建、远端仅存 scrypt 摘要且上传/查看
>   职责隔离已演练；当时**部署助手无自动化轮换命令，凭证轮换未实现/未执行**。新证据（2026-09-11）：
>   已按人工授权用独立工具 `scripts/rotate-mobile-readonly.py rotate` 执行一次真实凭证轮换
>   （见部署文档 §10）：旧查看/上传凭证失效、新凭证生效、职责隔离与 TLS 正常。**仍待**轮换后
>   Windows 自动发布与实体手机用新密码恢复的客户端确认，故本项不勾选；原任务文字不因进展而削弱。
> - 9.4 已勾选（2026-09-11，基于用户实机确认）。历史（2026-09-09）：生产容器/独立证书/首份**受管空冒烟**
>   （CAS 0→V1、空集合、受管 SMOKE_ID）已执行；当时 Windows 首次真实业务发布与实体手机浏览器验收尚未执行。
>   新证据（2026-09-11）：用户确认 Windows 首次真实上传与实体手机读取已完成，真机 1–6 项全部通过
>   （三时间展示；前台检查相邻触发 ≤60s；断网保留已展示数据且完整刷新不可用；联网恢复自动回前台；
>   PC 断网仍可查看；新版本自动更新详情一致）。用户未提供设备型号/浏览器/精确耗时，故不作量化实测声明。
>   生产当前为真实业务快照（不再是首份空冒烟 V1）。说明：`workbench.michaelli.site` 的 A 记录自始即为
>   独立解析，本 change 未做 DNS 切换，也不存在「旧服务迁移」；已验收的 9.1/9.2 DNS/证书事实未改动。
> - 9.5 已勾选（部署负责人确认 diff 边界）：未改动 `openspec/specs/`，未触碰 `docs/issue/`、
>   `docs/training/vibe-coding/` 与真实客户文件。
>
> 进度：45/46 已勾选（仅 9.3 待轮换后的客户端恢复确认）。
