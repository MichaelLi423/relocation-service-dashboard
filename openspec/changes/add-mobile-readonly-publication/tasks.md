...
> 进度说明（2026-09-09；实际生产证据见 `docs/mobile-readonly-deployment.md` §8/§9，
> 勾选只代表「该步已完成或经部署负责人确认」的事实，不代表 OpenSpec 生产发布整体完结）：
> - 9.1 已勾选：定向解析/HTTP-01 challenge 验证通过（host=容器=外部一致，80→308→HTTPS），
...
> - 9.3 保持未勾选：本机 Keychain 两条目已建、远端仅存 scrypt 摘要且上传/查看职责隔离已演练；
>   **桌面上传 token 的一次性配置与 Windows 生产发布尚未完成**；凭证**轮换未实现/未执行**
>   （部署助手无自动化轮换命令且未演练；服务本身可经人工授权的 digest 替换 + 自有服务重启轮换，
>   本文不给未受保护的现成命令；不覆盖既有 digest、不把密钥写入 argv/chat/仓库）。
> - 9.4 保持未勾选：生产容器/独立证书/首份**空冒烟**已执行（CAS 0→V1、空集合、受管 SMOKE_ID）；
>   Windows 首次真实业务发布与**实体手机**浏览器验收尚未执行（不称全部 9.x 完成）。
> - 9.5 保持已勾选（部署负责人确认 diff 边界）。
